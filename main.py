...
#profile-view,#public-profile-view{padding:20px;display:flex;flex-direction:column;align-items:center;text-align:center}
.profile-pic-lg{width:130px;height:130px;border-radius:50%;object-fit:cover;border:3px solid var(--primary);margin:10px auto;box-shadow:0 0 20px rgba(102,252,241,0.15)}
...
<div id="emoji-picker"><div class="emoji-header"><span style="color:var(--primary);font-weight:bold;font-family:'Rajdhani'">EMOJIS</span><span onclick="toggleEmoji()" style="color:#ff5555;cursor:pointer;font-weight:bold">✕</span></div><div id="emoji-grid" class="emoji-grid"></div></div>
...
<div id="app">
    ...
    <div id="content-area">
        ...
        <div id="view-profile" class="view">
            <div style="display:flex;flex-direction:column;align-items:center;width:100%;text-align:center;"><img id="p-avatar" src="" class="profile-pic-lg" onclick="document.getElementById('modal-profile').classList.remove('hidden')"><h2 id="p-name" style="color:white;font-family:'Rajdhani';width:100%;text-align:center;margin:10px 0;">...</h2><p id="p-bio" style="color:#888;width:100%;text-align:center;margin:0 0 20px 0;font-style:italic">...</p></div>
            <div id="search-box"><input id="search-input" placeholder="Buscar Soldado..."><button onclick="searchUsers()" style="background:none;border:none;color:var(--primary);font-size:18px;cursor:pointer">🔍</button></div><div id="search-results"></div>
            <div style="display:flex;justify-content:center;gap:10px;width:95%;max-width:320px;margin:10px auto;"><button onclick="toggleRequests('requests')" class="btn-std">📩 Solicitações</button><button onclick="toggleRequests('friends')" class="btn-std">👥 Amigos</button></div>
            <div id="requests-list" style="display:none;width:95%;max-width:320px;margin:10px auto"></div>
            <div style="display:flex;justify-content:center;width:95%;max-width:320px;margin:30px auto 0 auto;"><button onclick="logout()" class="btn-std" style="border-color:#ff5555;color:#ff5555;width:100%;text-align:center;">SAIR DO SISTEMA</button></div>
        </div>
...
const EMOJIS = ["😂","🔥","❤️","💀","🎮","🇧🇷","🫡","🤡","😭","😎","🤬","👀","👍","👎","🔫","💣","⚔️","🛡️","🏆","💰","🍕","🍺","👋","🚫","✅","👑","💩","👻","👽","🤖","🤫","🥶","🤯","🥳","🤢","🤕","🤑","🤠","😈","👿","👹","👺","👾"];
window.onload=()=>{let g=document.getElementById('emoji-grid');EMOJIS.forEach(e=>{let s=document.createElement('div');s.className='emoji-btn';s.innerText=e;s.onclick=()=>addEmoji(e);g.appendChild(s)})};
...
async function openPublicProfile(uid){let r=await fetch('/user/'+uid+'?viewer_id='+user.id);let d=await r.json();document.getElementById('pub-avatar').src=d.avatar_url;document.getElementById('pub-name').innerText=d.username;document.getElementById('pub-bio').innerText=d.bio;let ab=document.getElementById('pub-actions');ab.innerHTML='';if(d.friend_status==='friends')ab.innerHTML='<span style="color:#0f0">✔ Amigos</span>';else if(d.friend_status==='pending_sent')ab.innerHTML='<span style="color:orange">Solicitado</span>';else if(d.friend_status==='pending_received')ab.innerHTML=`<button class="btn-std" onclick="handleReq(${d.request_id},'accept')">Aceitar</button>`;else ab.innerHTML=`<button class="btn-std" onclick="sendRequest(${uid})" style="border-color:var(--primary)">Adicionar</button>`;let g=document.getElementById('pub-grid');g.innerHTML='';d.posts.forEach(p=>{g.innerHTML+=p.media_type==='video'?`<video src="${p.content_url}" class="grid-item" controls preload="none"></video>`:`<img src="${p.content_url}" class="grid-item" loading="lazy">`});goView('public-profile')}
...
async function toggleRequests(type){let b=document.getElementById('requests-list');if(b.style.display==='block'&&b.dataset.type===type){b.style.display='none';return}b.style.display='block';b.dataset.type=type;b.innerHTML='Carregando...';let d=await (await fetch('/friend/requests?uid='+user.id)).json();b.innerHTML='';if(type==='requests'){b.innerHTML='<h4 style="color:#888;margin:5px 0">Recebidas</h4>';if(!d.requests.length)b.innerHTML+='<small>Nenhuma.</small>';d.requests.forEach(r=>{b.innerHTML+=`<div class="search-res" style="background:rgba(255,255,255,0.05)"><span>${r.username}</span><div><button onclick="handleReq(${r.id},'accept')" style="color:#0f0;background:none;border:none;cursor:pointer">✔</button><button onclick="handleReq(${r.id},'reject')" style="color:red;background:none;border:none;cursor:pointer">✖</button></div></div>`})}else{b.innerHTML='<h4 style="color:#888;margin:5px 0">Meus Amigos</h4>';if(!d.friends.length)b.innerHTML+='<small>Nenhum.</small>';d.friends.forEach(f=>{b.innerHTML+=`<div class="search-res" onclick="openPublicProfile(${f.id})"><img src="${f.avatar_url}" style="width:25px;height:25px;border-radius:50%"> ${f.username}</div>`})}}
...
function submitPost(){let f=document.getElementById('file-upload').files[0];let cap=document.getElementById('caption-upload').value;if(!f)return showToast("Arquivo?");let blob=URL.createObjectURL(f);let m=f.type.startsWith('video')?`<video src="${blob}" class="post-media" controls></video>`:`<img src="${blob}" class="post-media">`;let html=`<div class="post-card" style="opacity:0.7"><div class="post-header"><div style="display:flex;align-items:center"><img src="${user.avatar_url}" class="post-av"><b>${user.username}</b></div><span>Enviando...</span></div>${m}<div class="post-caption"><b>${user.username}</b> ${cap}</div></div>`;document.getElementById('feed-container').insertAdjacentHTML('afterbegin',html);closeUpload();let fd=new FormData();fd.append('file',f);fd.append('user_id',user.id);fd.append('caption',cap);let x=new XMLHttpRequest();x.open('POST','/upload/post',true);x.onload=()=>{if(x.status===200){showToast("Enviado!");lastFeedHash="";loadFeed()}else showToast("Erro")};x.send(fd)}
function sendMsg(){let i=document.getElementById('chat-msg');let txt=i.value.trim();if(!txt)return;let b=document.getElementById('chat-list');let html=`<div class="msg-row mine temp"><img src="${user.avatar_url}" class="msg-av"><div><div class="msg-bubble">${txt}</div></div></div>`;b.insertAdjacentHTML('beforeend',html);b.scrollTop=b.scrollHeight;ws.send(txt);i.value='';toggleEmoji(true)}
async function uploadChatImage(){let f=document.getElementById('chat-file').files[0];if(!f)return;let blob=URL.createObjectURL(f);let b=document.getElementById('chat-list');let html=`<div class="msg-row mine temp"><img src="${user.avatar_url}" class="msg-av"><div><div class="msg-bubble"><img src="${blob}" class="chat-img"></div></div></div>`;b.insertAdjacentHTML('beforeend',html);b.scrollTop=b.scrollHeight;let fd=new FormData();fd.append('file',f);let r=await fetch('/upload/chat',{method:'POST',body:fd});if(r.ok){let d=await r.json();ws.send(d.url)}}
function connectWS(){if(ws)ws.close();let p=location.protocol==='https:'?'wss:':'ws:';ws=new WebSocket(`${p}//${location.host}/ws/Geral/${user.id}`);ws.onmessage=e=>{let d=JSON.parse(e.data);if(d.user_id===user.id){let t=document.querySelectorAll('.msg-row.mine.temp');if(t.length>0)t[t.length-1].remove()}let b=document.getElementById('chat-list');let m=d.user_id===user.id;let c=d.content.includes('/static/')?`<img src="${d.content}" class="chat-img" onclick="window.open(this.src)" loading="lazy">`:d.content;let a=`${d.avatar}?t=${new Date().getTime()}`;let html=`<div class="msg-row ${m?'mine':''}"><img src="${a}" class="msg-av" onclick="openPublicProfile(${d.user_id})"><div><div style="font-size:11px;color:var(--primary);margin-bottom:2px;cursor:pointer" onclick="openPublicProfile(${d.user_id})">${d.username}</div><div class="msg-bubble">${c}</div></div></div>`;b.insertAdjacentHTML('beforeend',html);b.scrollTop=b.scrollHeight};ws.onclose=()=>setTimeout(connectWS,2000)}
...
async def reg(d: RegisterData, db: Session=Depends(get_db)):
    if db.query(User).filter(User.username==d.username).first(): raise HTTPException(400, "Existe")
...
async def send_req(d: dict, db: Session=Depends(get_db)):
    sender_id, target_id = d.get('sender_id'), d.get('target_id')
    me, target = db.query(User).filter(User.id==sender_id).first(), db.query(User).filter(User.id==target_id).first()
    if target in me.friends: return {"status": "already_friends"}
    if db.query(FriendRequest).filter(or_(and_(FriendRequest.sender_id==sender_id, FriendRequest.receiver_id==target_id),and_(FriendRequest.sender_id==target_id, FriendRequest.receiver_id==sender_id))).first(): return {"status": "pending"}
...
async def get_reqs(uid: int, db: Session=Depends(get_db)):
    ...
    friends_data = [{"id": f.id, "username": f.username, "avatar_url": f.avatar_url} for f in db.query(User).filter(User.id == uid).first().friends]
...
async def get_user_profile(target_id: int, viewer_id: int, db: Session=Depends(get_db)):
    ...
    posts = [{"content_url": p.content_url, "media_type": p.media_type} for p in db.query(Post).filter(Post.user_id == target_id).order_by(Post.timestamp.desc()).all()]
    status = "friends" if target in viewer.friends else "none"
    req_id = None
    if status == "none":
        if db.query(FriendRequest).filter(FriendRequest.sender_id == viewer_id, FriendRequest.receiver_id == target_id).first(): status = "pending_sent"
        if r := db.query(FriendRequest).filter(FriendRequest.sender_id == target_id, FriendRequest.receiver_id == viewer_id).first(): status = "pending_received"; req_id = r.id
    return {"username": target.username, "avatar_url": target.avatar_url, "bio": target.bio, "posts": posts, "friend_status": status, "request_id": req_id}
