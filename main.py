...
// --- CORREÇÃO DA FOTO (CACHE BUSTER) ---
function updateUI(){
    let t = new Date().getTime();
    let url = user.avatar_url + "?t=" + t;
    // Atualiza TODAS as fotos na tela de uma vez
    document.querySelectorAll('.my-avatar-mini, .profile-pic-lg, .msg-row.mine .msg-av').forEach(img => {
        img.src = url;
    });
...
function connectWS(){
    ...
    ws.onmessage=e=>{
        ...
        // Remove mensagens temporarias se for minha msg
...
function closeUpload(){
    ...
    document.getElementById('file-upload').value = ""; // RESET DO INPUT PC
    toggleEmoji(true);
}
...
async function updateProfile(){
    ...
    if(r.ok){
        ...
        user.bio=d.bio;
        updateUI(); // FORÇA A MUDANÇA NA HORA
