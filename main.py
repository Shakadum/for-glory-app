...
class User(Base):
    ...
    xp = Column(Integer, default=0)  # Sistema de XP
...
.modal{position:fixed;inset:0;background:rgba(0,0,0,0.6);z-index:9000;display:flex;align-items:center;justify-content:center;backdrop-filter:blur(8px)}
.modal-box{background:rgba(20,25,35,0.95);padding:30px;border-radius:24px;border:1px solid var(--border);width:90%;max-width:380px;text-align:center;box-shadow:0 20px 50px rgba(0,0,0,0.5);animation:scaleUp 0.3s cubic-bezier(0.175, 0.885, 0.32, 1.275)}
...
<div id="modal-upload" class="modal hidden"><div class="modal-box"><h2 style="color:white;font-family:'Rajdhani'">NOVO ARQUIVO</h2><input type="file" id="file-upload" class="inp" accept="image/*,video/*" style="text-align:left"><div style="display:flex;gap:5px;align-items:center;margin-bottom:10px"><input type="text" id="caption-upload" class="inp" placeholder="Legenda da Missão..." style="margin:0;text-align:left"><button onclick="openEmoji('caption-upload')" style="background:none;border:none;font-size:22px;cursor:pointer">😀</button></div><button onclick="submitPost()" class="btn-main">PUBLICAR (+50 XP)</button><button onclick="closeUpload()" style="width:100%;padding:12px;margin-top:10px;background:transparent;border:1px solid #444;color:#888;border-radius:10px;cursor:pointer">CANCELAR</button></div></div>
...
function updateUI(){let t=new Date().getTime();document.querySelectorAll('.my-avatar-mini, #p-avatar').forEach(img=>{img.src=user.avatar_url+"?t="+t;});document.getElementById('p-cover').src=(user.cover_url||"https://via.placeholder.com/600x200/0b0c10/66fcf1?text=FOR+GLORY")+"?t="+t;document.getElementById('p-name').innerText=user.username;document.getElementById('p-bio').innerText=user.bio;document.getElementById('p-rank').innerText=user.rank;}
...
function submitPost(){let f=document.getElementById('file-upload').files[0];let cap=document.getElementById('caption-upload').value;if(!f)return showToast("Selecione um arquivo!");let fd=new FormData();fd.append('file',f);fd.append('user_id',user.id);fd.append('caption',cap);let x=new XMLHttpRequest();x.open('POST','/upload/post',true);x.onload=()=>{if(x.status===200){showToast("Publicado! +50 XP");user.xp+=50;lastFeedHash="";loadFeed();closeUpload();fetch('/login',{method:'POST',body:JSON.stringify({username:user.username,password:''})}).then(r=>r.json()).then(u=>{user=u;updateUI()});}else showToast("Falha no envio")};x.send(fd);}
...
function toggleEmoji(){let e=document.getElementById('emoji-picker');e.style.display=e.style.display==='flex'?'none':'flex'}
window.onload=()=>{let g=document.getElementById('emoji-grid');EMOJIS.forEach(e=>{let s=document.createElement('div');s.style.cssText="font-size:24px;cursor:pointer;text-align:center;padding:5px;";s.innerText=e;s.onclick=()=>{if(currentEmojiTarget)document.getElementById(currentEmojiTarget).value+=e};g.appendChild(s)})};
</script>
